...
│   │   ├── escalafones/
│   │   ├── usuarios/
│   │   ├── roles/            ← RBAC dinámico (Sprint 8)
│   │   └── permisos/         ← Catálogo de permisos (Sprint 8)
│   ├── shared/
│   │   ├── middleware/
│   │   │   ├── auth.middleware.ts
│   │   │   ├── permisos.middleware.ts  ← Reemplaza roles.middleware.ts (Sprint 8)
│   │   │   └── audit.middleware.ts
│   │   ├── errors/
│   │   ├── codigoCargo.ts
│   │   ├── prisma.ts
...
- `requirePermiso('bajas:write')` reemplaza `requireRole(['admin', 'editor'])`
