...
> Última actualización: 2026-09-30 (Sprint 18 designación + retenciones ✅; Sprint 19 designación CPH ✅; Sprint 19 retenciones — backend ✅ / frontend en curso; gráfico KPIs evolución de dotación ✅)
>
...
| Sprint 18 — Designación CPH: búsqueda mejorada + validación especialidad + ID SIAL Rol                     | ✅ Completado — integrado en deploy (commit `8dfec64`)                                                                                                                                                                                                                                                | S18-1 a S18-8                | `Sprints/SPRINT_18_designacion_cph_especialidad.md`                |
...
| Sprint 18 — Retenciones: schema + generación automática de cargos R/TTR                                    | ✅ Completado — módulo `retenciones/` (registrar R/TTR, cadena, titular-cesa, validación)                                                                                                                                                                                                             | S18-1 a S18-10               | `Sprints/SPRINT_18_retenciones_cadena_cargos.md`                   |
| Sprint 19 — Designación CPH Etapa 5: estado completo + validación contra padrón                            | ✅ Completado — integrado en deploy (commit `8dfec64`): `getDesignacionEstadoService`, flag `validado`, panel Etapa 5, filtros                                                                                                                                                                        | S19-1 a S19-7                | `Sprints/SPRINT_19_designacion_cph_estado_completo.md`             |
| Sprint 19 — Retenciones: vencimientos, renovación, comisión y vista de conducción                          | 🟡 En curso — backend ✅ (enum vencimiento, `materializarAlertasVencimiento`, `renovarPeriodo`, módulo `comisiones/`; 6/6 e2e OK, sin commitear) · frontend Agustín pendiente (S19-9/10/11)                                                                                                           | S19-1 a S19-13               | `Sprints/SPRINT_19_retenciones_vencimientos_comision.md`           |
| Sprint 20 — Consolidación catálogo Puesto/Escalafón/Especialidad + limpieza BD dev                         | 📋 Planificado — **arranca recién cuando Sprint 18 (Designación CPH), Sprint 18 (Retenciones) y Sprint 19 estén completos**; bloquea la carga de datos reales/producción hasta que cierre                                                                                                             | S20-1 a S20-13               | `Sprints/SPRINT_20_consolidacion_puesto_escalafon_especialidad.md` |
| Sprint 21 — Consumidores secundarios de puesto/especialidad + documentos exportables                       | 📋 Planificado — depende de Sprint 20 ✅                                                                                                                                                                                                                                                              | S21-1 a S21-11               | `Sprints/SPRINT_21_consumidores_secundarios_especialidad.md`       |
| Sprint 22 — Dotaneitor resuelve contra el catálogo (Puesto/Especialidad)                                   | 📋 Planificado — depende de Sprint 20 ✅ y Sprint 21 ✅                                                                                                                                                                                                                                               | S22-1 a S22-9                | `Sprints/SPRINT_22_dotaneitor_catalogo.md`                         |
...
- **(2026-09-02, Agustin) `ref_agrupadores` / `ref_unificadores_puesto` / `ref_especialidades_cuil` migradas por S2-19 pero nunca cargadas — el padrón salía con `AGRUPADOR`/`UNIFICADOR DE PUESTOS`/`UNIVERSO TOTALIZADOR`/`MONOVALENCIA` vacíos para el 100% de las filas, sin ningún error.** Diagnóstico y fix completos en `Doc/Dotaneitor_Analisis.md` sección 9. ✅ **Resuelto en mi Postgres local** con el script nuevo `services/dotaneitor/scripts/seed_referencias.py`.
  ✅ **Jorge llegó al mismo hallazgo por su cuenta (2026-09-04, antes de leer esta nota) y lo resolvió en su entorno** con su propio script `scripts/migrar_ref_dotacion.py` (mismo objetivo, escrito en paralelo) — de paso también cerró el resto del paso 14 de `Dotaneitor_Analisis.md` (las 7 tablas `ref_abreviaturas_*`/`ref_correcciones_*`/etc., antes creadas pero vacías) con una migración portable de verdad (`20260910000000_seed_ref_dotaneitor`), y migró los campos de `hospitales` (`20260908000000_hospitales_enriquecer`) — esos dos sí viajan solos con el merge, a cualquier entorno.
  **Sigue pendiente solo para `ref_agrupadores`/`ref_unificadores_puesto`/`ref_especialidades_cuil`:** ninguno de los dos scripts es una migración — siguen siendo un paso manual que hay que correr a mano en cada entorno nuevo (staging/producción cuando se levanten). Cualquiera de los dos sirve, no hace falta correr ambos:
  ```
  cd services/dotaneitor
  python scripts/seed_referencias.py --archivo "/ruta/a/ARCHIVOS PARA DOTACION.xlsx"
  ```
  Necesitás tu propia copia de `ARCHIVOS PARA DOTACION.xlsx` (no está en el repo, tiene CUIL/nombres reales). Es re-ejecutable sin riesgo (trunca y recarga las `ref_*`; el update de `hospitales` no crea filas nuevas, aunque ya es redundante con la migración de Jorge).
